...
/*				ret = my_compress(&argv[2], output_file, Z_DEFAULT_COMPRESSION);
				if (ret != Z_OK)
					my_errors(ret);
				return ret;
*/
...
/*
				ret = my_decompress(&argv[2], output_file);
				if (ret != Z_OK)
					my_errors(ret);
				return ret;
*/
		...
		case 4:
			...
			if (strcmp(argv[1], "-c") == 0){
				ret = my_compress((FILE *)input_file, (FILE *)output_file, Z_DEFAULT_COMPRESSION);
			}else if (strcmp(argv[1], "-d") == 0){
				ret = my_decompress((FILE *)input_file, (FILE *)output_file);
			}else{
				usage(argv[0]);
				return UNSUCCESS;
			}
