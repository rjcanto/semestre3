...
int my_compress(FILE *source, FILE *dest, int level, myFilter myfilter)
{
    ...
    unsigned have;
    z_stream strm;
    unsigned char in[myfilter.lineSize+1];
    unsigned char out[myfilter.lineSize+1];
    unsigned char prev[myfilter.lineSize+1];
	unsigned char aux_header[myfilter.nbrChars];
    ...
    do {
		...
		if (myfilter.firstLine){
			...
			strm.avail_in = fread(in, 1, CHUNK, source)+1;
...
int main(int argc, char **argv)
{
	 ...
	 **/
	myfilter.nbrChars=processFileHeader(myargs.source,&fhp);
